...
function isBlockScopeBoundary(node) {
  switch (node.kind) {
    case ts18.SyntaxKind.Block: {
      const parent = node.parent;
      return parent.kind !== ts18.SyntaxKind.CatchClause && // blocks inside SourceFile are block scope boundaries
      (parent.kind === ts18.SyntaxKind.SourceFile || // blocks that are direct children of a function scope boundary are no scope boundary
      // for example the FunctionBlock is part of the function scope of the containing function
      !isFunctionScopeBoundary(parent)) ? 2 /* Block */ : 0 /* None */;
    }
    case ts18.SyntaxKind.ForStatement:
    case ts18.SyntaxKind.ForInStatement:
    case ts18.SyntaxKind.ForOfStatement:
    case ts18.SyntaxKind.CaseBlock:
    case ts18.SyntaxKind.CatchClause:
    case ts18.SyntaxKind.WithStatement:
      return 2 /* Block */;
    default:
      return 0 /* None */;
  }
...
function identifierToKeywordKind(node) {
  return "identifierToKeywordKind" in ts19 ? ts19.identifierToKeywordKind(node) : (
    // eslint-disable-next-line deprecation/deprecation
    node.originalKeywordKind
  );
}
function canHaveDecorators(node) {
  return "canHaveDecorators" in ts19 ? ts19.canHaveDecorators(node) : "decorators" in node;
}
function getDecorators(node) {
  return "getDecorators" in ts19 ? ts19.getDecorators(node) : node.decorators;
}
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    case ts20.SyntaxKind.TypeParameter:
    case ts20.SyntaxKind.InterfaceDeclaration:
    case ts20.SyntaxKind.TypeAliasDeclaration:
      return 2 /* Type */;
    case ts20.SyntaxKind.ClassDeclaration:
    case ts20.SyntaxKind.ClassExpression:
      return 2 /* Type */ | 4 /* Value */;
    case ts20.SyntaxKind.EnumDeclaration:
      return 7 /* Any */;
    case ts20.SyntaxKind.NamespaceImport:
    case ts20.SyntaxKind.ImportClause:
      return 7 /* Any */ | 8 /* Import */;
    case ts20.SyntaxKind.ImportEqualsDeclaration:
    case ts20.SyntaxKind.ImportSpecifier:
      return node.parent.name === node ? 7 /* Any */ | 8 /* Import */ : void 0;
    case ts20.SyntaxKind.ModuleDeclaration:
      return 1 /* Namespace */;
    case ts20.SyntaxKind.Parameter:
      if (node.parent.parent.kind === ts20.SyntaxKind.IndexSignature || identifierToKeywordKind(node) === ts20.SyntaxKind.ThisKeyword) {
        return;
      }
    case ts20.SyntaxKind.BindingElement:
    case ts20.SyntaxKind.VariableDeclaration:
      return node.parent.name === node ? 4 /* Value */ : void 0;
    case ts20.SyntaxKind.FunctionDeclaration:
    case ts20.SyntaxKind.FunctionExpression:
      return 4 /* Value */;
  }
}
...
function unwrapParentheses(node) {
  while (node.kind === ts21.SyntaxKind.ParenthesizedExpression) {
    node = node.expression;
  }
  return node;
}
function getPropertyName(propertyName) {
  if (propertyName.kind === ts21.SyntaxKind.ComputedPropertyName) {
    const expression = unwrapParentheses(propertyName.expression);
    if (ts21.isPrefixUnaryExpression(expression)) {
      let negate = false;
      switch (expression.operator) {
        case ts21.SyntaxKind.MinusToken:
          negate = true;
        case ts21.SyntaxKind.PlusToken:
          return ts21.isNumericLiteral(expression.operand) ? `${negate ? "-" : ""}${expression.operand.text}` : ts21.isBigIntLiteral(expression.operand) ? `${negate ? "-" : ""}${expression.operand.text.slice(0, -1)}` : void 0;
        default:
          return;
      }
    }
    if (ts21.isBigIntLiteral(expression)) {
  ...
  return propertyName.kind === ts21.SyntaxKind.PrivateIdentifier ? void 0 : propertyName.text;
}
...
function getUsageDomain(node) {
  const parent = node.parent;
  switch (parent.kind) {
    case ts22.SyntaxKind.TypeReference:
      return identifierToKeywordKind(node) !== ts22.SyntaxKind.ConstKeyword ? 2 /* Type */ : void 0;
    case ts22.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token === ts22.SyntaxKind.ImplementsKeyword || parent.parent.parent.kind === ts22.SyntaxKind.InterfaceDeclaration ? 2 /* Type */ : 4 /* Value */;
    case ts22.SyntaxKind.TypeQuery:
      return 5 /* ValueOrNamespace */ | 8 /* TypeQuery */;
    case ts22.SyntaxKind.QualifiedName:
      if (parent.left === node) {
        if (getEntityNameParent(parent).kind === ts22.SyntaxKind.TypeQuery) {
          return 1 /* Namespace */ | 8 /* TypeQuery */;
        }
        return 1 /* Namespace */;
      }
      break;
    case ts22.SyntaxKind.ExportSpecifier:
      if (parent.propertyName === void 0 || parent.propertyName === node) {
        return 7 /* Any */;
      }
      break;
    case ts22.SyntaxKind.ExportAssignment:
      return 7 /* Any */;
    case ts22.SyntaxKind.BindingElement:
      if (parent.initializer === node) {
        return 5 /* ValueOrNamespace */;
      }
      break;
    case ts22.SyntaxKind.Parameter:
    case ts22.SyntaxKind.EnumMember:
    case ts22.SyntaxKind.PropertyDeclaration:
    case ts22.SyntaxKind.VariableDeclaration:
    case ts22.SyntaxKind.PropertyAssignment:
    case ts22.SyntaxKind.PropertyAccessExpression:
    case ts22.SyntaxKind.ImportEqualsDeclaration:
      if (parent.name !== node) {
        return 5 /* ValueOrNamespace */;
      }
      break;
    case ts22.SyntaxKind.JsxAttribute:
    case ts22.SyntaxKind.FunctionDeclaration:
    case ts22.SyntaxKind.FunctionExpression:
    case ts22.SyntaxKind.NamespaceImport:
    case ts22.SyntaxKind.ClassDeclaration:
    case ts22.SyntaxKind.ClassExpression:
    case ts22.SyntaxKind.ModuleDeclaration:
    case ts22.SyntaxKind.MethodDeclaration:
    case ts22.SyntaxKind.EnumDeclaration:
    case ts22.SyntaxKind.GetAccessor:
    case ts22.SyntaxKind.SetAccessor:
    case ts22.SyntaxKind.LabeledStatement:
    case ts22.SyntaxKind.BreakStatement:
    case ts22.SyntaxKind.ContinueStatement:
    case ts22.SyntaxKind.ImportClause:
    case ts22.SyntaxKind.ImportSpecifier:
    case ts22.SyntaxKind.TypePredicate:
    case ts22.SyntaxKind.MethodSignature:
    case ts22.SyntaxKind.PropertySignature:
    case ts22.SyntaxKind.NamespaceExportDeclaration:
    case ts22.SyntaxKind.NamespaceExport:
    case ts22.SyntaxKind.InterfaceDeclaration:
    case ts22.SyntaxKind.TypeAliasDeclaration:
    case ts22.SyntaxKind.TypeParameter:
    case ts22.SyntaxKind.NamedTupleMember:
      break;
    default:
      return 5 /* ValueOrNamespace */;
  }
...
function getEntityNameParent(name) {
  ...
  while (parent.kind === ts22.SyntaxKind.QualifiedName) {
